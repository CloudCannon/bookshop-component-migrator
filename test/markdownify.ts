...
test("Markdownify empty check", (t: ExecutionContext) => {
	t.true(isMarkdownElement({ nodeName: '#text' }), `isMarkdownElement on "#text" nodes should return true`);
	t.true(isMarkdownElement({ localName: 'p' }), `isMarkdownElement on "p" nodes without attributes should return true`);
	t.false(isMarkdownElement({}), `isMarkdownElement on "{}"  should return false`);
});
